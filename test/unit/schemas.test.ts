...
describe("SubagentParams schema", { skip: !schemasAvailable ? "typebox not available" : undefined }, () => {
	...
	it("validates representative flexible field values with TypeBox compiler", { skip: !CompileSchema ? "typebox compiler not available" : undefined }, () => {
		...
		const validValues = [
			{ skill: "review" },
			{ skill: false },
			{ tasks: [{ agent: "delegate", task: "check this", reads: false }] },
			{ tasks: [{ agent: "delegate", task: "check this", skill: "review" }] },
			{ tasks: [{ agent: "delegate", task: "check this", skill: false }] },
			{ tasks: [{ agent: "delegate", task: "check this", output: "review.md", reads: ["input.md"], progress: true }] },
			{ chain: [{ agent: "delegate", reads: false }] },
			{ chain: [{ agent: "delegate", skill: "review" }] },
			{ chain: [{ agent: "delegate", skill: false }] },
			{ chain: [{ parallel: [{ agent: "delegate", reads: false, skill: false }] }] },
			{ chain: [{ parallel: [{ agent: "delegate", output: "review.md", reads: ["input.md"], skill: "review" }] }] },
			{ config: { name: "delegate", description: "Review things" } },
			{ config: JSON.stringify({ name: "delegate", description: "Review things" }) },
		];
		const invalidValues = [
			{ skill: 123 },
			{ skill: [123] },
			{ output: 123 },
			{ tasks: [{ agent: "delegate", task: "check this", reads: "input.md" }] },
			{ chain: [{ parallel: [{ agent: "delegate", output: 123 }] }] },
			{ chain: [{ parallel: [{ agent: "delegate", reads: "input.md" }] }] },
			{ chain: [{ parallel: [{ agent: "delegate", skill: 123 }] }] },
			{ config: [] },
			{ config: null },
		];
